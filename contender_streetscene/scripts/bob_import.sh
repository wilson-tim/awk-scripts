...
set_variables ()
{
	if [ ! "$DBPATH" ]
	then
		export DBPATH=/startrek/databases/version6/UPRN
	fi

	if [ ! "$IMPORTDIR" ]
	then
		export IMPORTDIR=/startrek/databases/v6/UPRN/crm_import
	fi

	if [ ! "$CCTDIR" ]
	then
		export CCTDIR=/startrek/usr/universe/version6/standard
	fi
	...
	if [ ! "$INFORMIXDIR" ]
	then
		export INFORMIXDIR=/startrek/usr/informix
	fi

	if [ ! "$INFORMIXSERVER" ]
	then
		export INFORMIXSERVER=scodev
	fi

	if [ ! "$FGLDBPATH" ]
	then
		export FGLDBPATH=/startrek/usr/universe/version6/standard/4jschema
	fi

	if [ ! "$FGLLDPATH" ]
	then
		export FGLLDPATH=/startrek/usr/universe/version6/standard/modules
	fi
	...
	if [ ! "$TERMCAP" ]
	then
		export TERMCAP=/startrek/usr2/bin/termcap
	fi
	...
	if [ ! "$DBTEMP" ]
	then
		export DBTEMP=/startrek/usr4/INFORMIX_TEMP
	fi
	...
	if [ ! "$PATH" ]
	then
		export PATH=$INFORMIXDIR/bin:$FGLDIR/bin:/startrek/usr/bin:$HOME/bin:$PATH
	fi
}
