case $# in
0 )
	echo "\nUsage nlpg_import.sh importfile"
	;;
* )
	...
	then
		...
		grep "^11" $1 | sed -e "s/,\ /+++++\ /g" | sed -e "s/,\([A-Z]\)/+++++\1/g" | sed -e "s/,/\|/g" | sed -e "s/+++++/,/g" > "$importfile"_11.unl
		if [ -f "$importfile"_11.unl ]
		then
			echo "importing"`wc -l $importfile"_11.unl`" files"
			popnum "$importfile"_11.unl "$importfile"_11a.unl
			...
			then
				...
				rm "$importfile"_11a.unl_1.unl
			fi
			if [ -f "$importfile"_11a.unl_2.unl ]
			then
				fglgo_gis_site_new debug "$importfile"_11a.unl_2.unl
				rm "$importfile"_11a.unl_2.unl
			fi
			rm "$importfile"_11.unl
			rm "$importfile"_11a.unl
		else
			echo "No location entries to import"
		fi

		echo "Processing nlpg24_site unload"
		grep "^24" $1 | sed -e "s/,\ /+++++\ /g" | sed -e "s/,\([A-Z]\)/+++++\1/g" | sed -e "s/,/\|/g" | sed -e "s/+++++/,/g" > "$importfile"_24.unl
		if [ -f "$importfile"_24.unl ]
		then
			echo "importing"`wc -l $importfile"_24.unl`" files"
			popnum "$importfile"_24.unl "$importfile"_24a.unl
			...
			then
				...
				rm "$importfile"_24a.unl_1.unl
			fi
			rm "$importfile"_24.unl
			rm "$importfile"_24a.unl
		fi

		echo "Processing nlpg21_site_detail unload"
		grep "^21" $1 | sed -e "s/,\ /+++++\ /g" | sed -e "s/,\([A-Z]\)/+++++\1/g" | sed -e "s/,/\|/g" | sed -e "s/+++++/,/g" > "$importfile"_21.unl
		if [ -f "$importfile"_21.unl ]
		then
			popnum "$importfile"_21.unl "$importfile"_21a.unl
			fglgo_nlpg21_site_detail debug "$importfile"_21a.unl
			fglgo_gis_site_new debug "$importfile"_21a.unl_1.unl
			rm "$importfile"_21a.unl
			rm "$importfile"_21a.unl_1.unl
		fi
	else
