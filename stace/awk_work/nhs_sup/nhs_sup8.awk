...
###############################################################################
#                                                                             #
# nhs_sup.awk script VERSION 0.08                                             #
...
# ????nhs.csv, and loads the data into the $DBPATH database, in the table     #
# nhs_sup. The program creates a temporay file $PHABASE/tmp/nhs_sup.tmp which #
# is removed when the load is complete. The $DBPATH/raw_data/????nhs.csv file #
# is also deleted on a successful load.                                       #
# The script also retrieves the nhs transaction number from the largest       #
# transaction number in the nhs_sup table.                                    #
...
BEGIN {
   ...
   while ((Cmd | getline) > 0) {
      ...
      if (truncate(SqlOutput[SqlOutSz - 2]) != 0 ) { 
         ...
         sql("lines 0; select max(transaction_no) from nhs_sup;")
      ...
      if (DoneSection == 1) {
         ...
         # If the nhs_sup table is blank then set the transaction number to 0
         if (NhsTran == "**********") { NhsTran = 0 }
      ...
      if (DoneSection == 3) {
         print ">>" NhsAsc " successfully imported."
         print ""
         system("rm " NhsAsc)
      }
