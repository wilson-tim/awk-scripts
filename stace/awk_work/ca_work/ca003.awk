###############################################################################
#                                                                             #
# Cellular Automata VERSION 0.03                                              #
   ...
   if (_dim == 1) {
      ...
      for (_count_y = 1; _count_y <= _size_y; _count_y++) {
         for (_count_x = 1; _count_x <= _size_x; _count_x++) {
            ...
            else if (_init_type == "test" ) {
               if (_count_x > 7 && _count_x < 11 && _count_y == 8) {
                  array[_count_x, _count_y] = 1
               }
               else {
                   array[_count_x, _count_y] = 0
               }
            }
         }
      }
...
_live, _born, _count_x, _count_y, _size_x, _size_y,   # Local Variables
_array_copy, _total, _xm1, _xp1, _ym1, _yp1) {   # Local Variables
   if (_dim == 1) {
      _live = substr(_rule, 1, index(_rule,"/") - 1)
      _born = substr(_rule, index(_rule,"/") + 1)
      _size_x = _size
...
# If nothing which is recognised is entered.
{ printf "dim[1-3] size[n] rule[live/born] generations[n]: "}
# -----------------------
