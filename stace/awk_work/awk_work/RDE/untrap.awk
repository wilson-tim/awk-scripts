###############################################################################
#                                                                             #
# Untrap Commitment VERSION 0.01                                              #
...
BEGIN {
   ...
   sql("lines 0; select order_number from oh_dets where order_status = 'I' and order_number in (select order_number from ol_dets where line_status = 'X');")
   ...
   if (SqlOutput[SqlOutSz - 2] == "There were no rows selected.") {
      ...
      for (Count = 9; Count <= OrdSqlOutSz - 2; Count++) {
         ...
         # Just select the 'C' order lines.
         sql("lines 0; select count(*) from ol_dets where order_number = " \
             OrdSqlOutput[Count] "and line_status = 'C';")
         CLinesCnt = truncate(SqlOutput[SqlOutSz - 2])

         # Just select the 'X' order lines.
         sql("lines 0; select count(*) from ol_dets where order_number = " \
             OrdSqlOutput[Count] "and line_status = 'X';")
         XLinesCnt = truncate(SqlOutput[SqlOutSz - 2])

         if ((CLinesCnt > 0) && (XLinesCnt > 0) && \
             (AllLinesCnt == CLinesCnt + XLinesCnt)) { 
            print "Updating (Closing) Order: " OrdSqlOutput[Count]
            sql("update oh_dets set order_status = 'C' where order_number = " \
                OrdSqlOutput[Count] ";") 
            OrdCnt++
         }
      }
      print OrdCnt " row(s) updated."
   }  
}
